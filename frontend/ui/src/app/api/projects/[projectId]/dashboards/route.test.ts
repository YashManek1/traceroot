...
describe("GET /dashboards — lazy seeding (no existing dashboards)", () => {
  ...
  it("includes a detector widget when a detector exists", async () => {
    dashboardFindManyMock.mockResolvedValueOnce([]).mockResolvedValueOnce([
      {
        id: "default_proj-1",
        name: "Overview",
    ...
    detectorFindFirstMock.mockResolvedValue({ id: "det-42" });
    dashboardCreateMock.mockResolvedValue({});

    await GET(makeGetRequest(), makeParams("proj-1"));

    const { data } = dashboardCreateMock.mock.calls[0][0] as {
      data: { widgets: { create: Array<{ type: string; spec: { detectorId: string } }> } };
    };
    const detectorWidgets = data.widgets.create.filter((w) => w.type === "detector");
    expect(detectorWidgets).toHaveLength(1);
    expect(detectorWidgets[0].spec.detectorId).toBe("det-42");
  });

  it("omits the detector widget when no detector exists", async () => {
    dashboardFindManyMock.mockResolvedValueOnce([]).mockResolvedValueOnce([
      {
        id: "default_proj-1",
        name: "Overview",
    ...
    const { data } = dashboardCreateMock.mock.calls[0][0] as {
      data: { widgets: { create: Array<{ type: string }> } };
    };
    const detectorWidgets = data.widgets.create.filter((w) => w.type === "detector");
