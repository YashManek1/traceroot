...
function QueryWidgetBody({
  ...
  if (!data) return null;
  return <QueryWidgetRenderer display={spec.display.type} result={data} />;
}
...
export function WidgetCard({
  ...
  const isDetector = widget.type === "detector";

  return (
    <div
      className={[
        "flex h-full flex-col rounded-md border bg-background p-3",
        isDetector ? "border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-950/30" : "",
      ]
        .join(" ")
        .trim()}
    >
