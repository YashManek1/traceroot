...
import type { DisplayType, WidgetQueryResult } from "../types";
...
function NumberView({ result }: { result: WidgetQueryResult }) {
  const value = result.rows[0]?.[result.columns.length - 1];
  return (
    <div className="flex h-full items-center justify-center">
      <span className="text-3xl font-semibold">{fmtNumber(value)}</span>
    </div>
  );
}
...
export function QueryWidgetRenderer({
  ...
  switch (display) {
    ...
    case "number":
      return <NumberView result={result} />;
    case "table":
