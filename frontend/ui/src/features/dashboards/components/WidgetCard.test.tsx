...
vi.mock("./DetectorWidget", () => ({
  DetectorWidget: (props: { projectId: string; spec: { detectorId: string } }) => (
    <div data-testid="detector-widget">
      {props.projectId}:{props.spec.detectorId}
    </div>
  ),
}));
...
describe("WidgetCard menu gating", () => {
  ...
  it("still shows Duplicate and Delete for a non-query widget", async () => {
    const { onDuplicate, onDelete } = renderCard(
      makeWidget({ type: "detector", spec: { detectorId: "d1" } }),
    );
...
describe("WidgetCard body dispatch", () => {
  ...
  it("dispatches detector widgets to DetectorWidget", () => {
    renderCard(makeWidget({ type: "detector", spec: { detectorId: "det-1" } }));

    const body = screen.getByTestId("detector-widget");
    expect(body.textContent).toBe("p1:det-1");
  });
});
